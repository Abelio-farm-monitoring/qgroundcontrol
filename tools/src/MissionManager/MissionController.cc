...
MissionController::MissionController(PlanMasterController *masterController, QObject *parent)
    : PlanElementController(masterController, parent), _planViewSettings(qgcApp()->toolbox()->settingsManager()->planViewSettings()), _missionManager(_managerVehicle->missionManager()), _missionItemCount(0), _visualItems(nullptr), _settingsItem(nullptr), _firstItemsFromVehicle(false), _itemsRequested(false), _inRecalcSequence(false), _appSettings(qgcApp()->toolbox()->settingsManager()->appSettings()), _progressPct(0), _currentPlanViewSeqNum(-1), _currentPlanViewVIIndex(-1), _currentPlanViewItem(nullptr), _splitSegment(nullptr)
{
...
void MissionController::convertToKMLDocument(QDomDocument &document)
{
    ...
    for (const auto &item : rgMissionItems)
    {
        ...
        if (uiInfo && uiInfo->specifiesCoordinate() && !uiInfo->isStandaloneCoordinate())
        {
            ...
            coord = QString::number(item->param6(), 'f', 7) + "," + QString::number(item->param5(), 'f', 7) + "," + QString::number(amslAltitude, 'f', 2);
            coords.append(coord);
        }
    }
